...
static unsigned long last_ttft_ms,last_total_ms;static unsigned int last_output_tokens;static int last_log_ok;
...
static void conversation_screen(const char *question,const char *answer,int generating,unsigned int tokens){char st[96];clear();title();set_pen(create_rgb16(255,255,0));render_text_jp(12,39,"質問");set_pen(create_rgb16(255,255,255));render_text_jp_wrapped_clipped(12,60,500,116,question);set_pen(create_rgb16(70,70,70));draw_line(12,119,516,119);set_pen(create_rgb16(255,255,0));render_text_jp(12,130,generating==2?"考えています":generating?"回答を生成中":"回答");set_pen(create_rgb16(255,255,255));render_text_jp_wrapped_clipped(12,153,500,generating?282:264,answer);if(generating){sprintf(st,"%u tokens / HISTORY: screenshot",tokens);set_pen(create_rgb16(0,255,255));render_text(12,292,st);}else{sprintf(st,"TTFT %ums / TOTAL %ums / %u tok / LOG %s",(unsigned int)last_ttft_ms,(unsigned int)last_total_ms,last_output_tokens,last_log_ok?"OK":"NG");set_pen(create_rgb16(0,255,255));render_text(12,272,st);set_pen(create_rgb16(0,255,0));render_text_jp(12,296,"決定:質問へ 戻る:終了 履歴:画面保存");}lcdc_copy_vram();}
...
int main(void *ptr){
 ...
 memmgr_init();graphics_init(SW,SH,(void*)0xAC200000);benchmark_init();
 ...
 for(;;){
  ...
  for(;;){
   ...
   if(get_key_state(KEY_ENTER)){
    unsigned int i,pos=0,n=0,generated=0;unsigned short tok,*prompt;unsigned int prompt_len;int first_token=1;
    ...
    draft[0]=0;last_ttft_ms=last_total_ms=0;last_output_tokens=0;last_log_ok=0;benchmark_begin();
    if(thinking){
    ...
    for(i=0;i<FINAL_MAX_TOKENS&&pos<CTX;i++,pos++){
     tok=greedy(&m);if(tok==EX_EOS)break;if(first_token){last_ttft_ms=benchmark_elapsed_ms();first_token=0;}generated++;append_token(out,&n,tok);forward(&m,tok,pos);
     if(utf8_complete(out)){conversation_screen(question,out,1,i+1);capture_if_requested();}
    }
    last_total_ms=benchmark_elapsed_ms();benchmark_end();last_output_tokens=generated;last_log_ok=benchmark_log(question,prompt_len,generated,thinking,last_ttft_ms,last_total_ms);
    conversation_screen(question,out[0]?out:"（回答を生成できませんでした）",0,generated);
    for(;;){if(capture_if_requested())continue;if(pressed(KEY_POWER)||pressed(KEY_BACK))return -2;if(get_key_state(KEY_ENTER)){release(KEY_ENTER);break;}}
    break;
   }
  }
